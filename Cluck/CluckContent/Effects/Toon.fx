...
float4 CelPixelShader(VertexToPixel input) : COLOR0
{
    ...
    float intensity = dot(normalize(DiffuseLightDirection), normalize(input.Normal));
    if(intensity < 0)
        intensity = 0;
 
    // Calculate what would normally be the final color, including texturing and diffuse lighting
    float4 color = (AmbientColor * AmbientIntensity) + (tex2D(textureSampler, input.TextureCoordinate) * DiffuseColor * DiffuseIntensity);
    color.a = 1;
...
VertexToPixel OutlineVertexShader(AppToVertex input)
{
    ...
    float4 original = mul(mul(input.Position, View), Projection);
